...
class UserController extends Controller
{
    ...
    public function receiverDashboard()
    {
        ...
        $donation_count = Donation::where('hidden', false)->where('approval', 1)->count();
        $active_count = Donation::where('hidden', false)->where('status', [0, 1, 2])->where('approval', 1)->count();
        $success_count = Donation::where('hidden', false)->where('status', 4)->where('approval', 1)->count();
        $wasted_count = Donation::where('hidden', false)->where('status', 3)->where('approval', 1)->count();
        return view('receiver.dashboard', compact('donation_count', 'active_count', 'success_count', 'wasted_count'));
    }

    public function donorDashboard()
    {
        $all = Donation::where('user_id', Auth::user()->id)->where('hidden', false)->orderBy('created_at', 'desc')->take(3)->get();
        return view('donor.dashboard', compact('all'));
    }
